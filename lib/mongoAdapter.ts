...
export function MongoDBAdapter(client: Promise<MongoClient>, options: MongoDBAdapterOptions = {}): Adapter {
    ...
    return {
        ...
        async deleteUser(id) {
            const userId = _id(id);
            const m = await db;
            await Promise.all([m.A.deleteMany({ userId }), m.S.deleteMany({ userId }), m.U.deleteOne({ _id: userId })]);
        },
